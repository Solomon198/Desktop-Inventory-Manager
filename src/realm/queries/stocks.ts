 ...
 * @param {productsFilterParams} - An object that contain the start and end date for filtering products
 * @returns {Promise<stocksResponse>} returns the total filtered products count and entities
//  */
function getFilterProducts(productsFilterParams: productsFilterParams) {
  return new Promise<getStocksResponse>((resolve, reject) => {
    try {
      let stocks: Realm.Results<Realm.Object>;
      stocks = app
        ...
        .filtered(
          "date >= $0 && date < $1",
          productsFilterParams.start_date,
          productsFilterParams.end_date
        );

      let partition = helperFuncs.getPaginationPartition(
        productsFilterParams.pageNumber,
