...
function getCustomer(customerId:string){
    return new Promise<CustomerProperties>((resolve,reject)=>{
      ...
      try{
        ...
        let customer = app.objectForPrimaryKey(Schemas.CustomerSchema.name,convertIdToObjectId as ObjectId);
    ...
    return new Promise<getCustomersResponse>((resolve,reject)=>{
        ...
        try{
          ...
          if(searchQuery.trim() && type.trim()){
            let query = 'first_name CONTAINS[c] $0 || last_name CONTAINS[c] $0 || email CONTAINS[c] $0 && cus_type == $1';
            customers = app.objects(Schemas.CustomerSchema.name).filtered(query,searchQuery,type);
          }else if(searchQuery.trim() && !type.trim()){
             
            let query = 'first_name CONTAINS[c] $0 || last_name CONTAINS[c] $0 || email CONTAINS[c] $0';
            customers = app.objects(Schemas.CustomerSchema.name).filtered(query,searchQuery);

          }else if(!searchQuery.trim() && type.trim()){
            let query = 'cus_type == $0';
            customers = app.objects(Schemas.CustomerSchema.name).filtered(query,type);
          }
          else {
...
function removeCustomer(customerId:string){
    ...
    return new Promise<boolean>((resolve,reject)=>{
        ...
        try{
           ...
           app.write(()=>{
            let customer = app.objectForPrimaryKey(Schemas.CustomerSchema.name,changeToObjectId as ObjectId );
            app.delete(customer);
            resolve(true);
           })
        }catch(e:any){
        ...
        try{
           ...
           app.write(()=>{
            ...
            changeToObjectIds.forEach((id)=>{
                let customer = app.objectForPrimaryKey(Schemas.CustomerSchema.name,id);
                app.delete(customer);
            })
 ...
 */
function updateCustomer(customer:CustomerProperties){
    customer._id = mongoose.Types.ObjectId(customer._id);
    return new Promise<CustomerProperties>((resolve,reject)=>{
            app.write(()=>{
            
                try{
                    let customerUpdate = app.create(Schemas.CustomerSchema.name,customer,Realm.UpdateMode.Modified);
