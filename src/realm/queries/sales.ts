    ...
    app.write(() => {
      try {
        ...
        saleObject.customer_name =
          typeof saleObject.customer_name === 'undefined'
            ? `Quick Sale`
  ...
  return new Promise<SaleProperties>((resolve, reject) => {
    try {
      ...
      if (Object.keys(customer).length !== 0) {
        ...
        saleObject.customer_name = 'XXXXXX XXXXXX';
        saleObject.customer_phone = 'XXXXXXXXXXX';
      }
      try {
        saleObject.date = helperFuncs.transformDateObjectToString(
          saleObject.date
    ...
    try {
      ...
      result.forEach((obj) => {
        ...
        if (Object.keys(customer).length !== 0) {
          ...
          newObj.customer_name = `XXXXX XXXXX`;
          newObj.customer_phone = `XXXXXXXXXXX`;
        }
    ...
    try {
      ...
      result.forEach((obj) => {
        ...
        newObj.customer_name =
          typeof newObj.customer_name === 'undefined'
            ? `Quick Sale`
            : customer.first_name + ' ' + customer.last_name;
        newObj.customer_phone = customer.phone_no;
        try {
