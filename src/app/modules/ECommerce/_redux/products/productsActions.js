...
export const fetchProductsForUnitManager = () => dispatch => {
  dispatch(actions.startCall({ callType: callTypes.list }));

  return ProductAPI.getProductsForUnitManager()
    .then(products => {
      let { totalCount, entities } = products;
      dispatch(actions.productsFetchedForUnitManager({ totalCount, entities }));
    })
    .catch(error => {
      dispatch(actions.catchError({ error, callType: callTypes.list }));
    });
