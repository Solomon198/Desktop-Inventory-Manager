...
export function CustomersCard(props) {
  ...
  const { customerForEdit, error, saleEntities } = useSelector((state) => ({
    customerForEdit: state.customers.customerForEdit,
    saleEntities: state.sales.entities,
    error: state.customers.error,
  }));

  let lastSaleId;

  if (saleEntities && saleEntities.length > 0) {
    lastSaleId = saleEntities[saleEntities.length - 1]._id;
  }
  ...
  const saveSale = async (values, resetForm) => {
    ...
    console.log('__customerTransaction', customerTransaction);

    console.log('__Finished sales', saveSale);
