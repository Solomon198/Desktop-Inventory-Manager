...
export function CustomerEditDialog({ id, show, onHide }) {
  ...
  const saveStock = (values, resetForm) => {
    if (!id) {
      ...
      dispatch(actions.createStock(_saveStock)).then(() => onHide());
      resetForm({ values: '' });
    } else {
      // server request for updating stock
      dispatch(actions.updateStock(values)).then(() => onHide());
    }
  };
  ...
  return (
    ...
    >
      ...
      <ul className="nav nav-tabs nav-tabs-line ml-5 " role="tablist">
        <li
          className="nav-item"
          onClick={() => customersUIProps.setTab('stock')}
        >
          <a
            className={`nav-link ${customersUIProps.tab === 'stock' &&
              'active'}`}
            data-toggle="tab"
            role="tab"
            aria-selected={(customersUIProps.tab === 'stock').toString()}
          >
            Add Stock
          </a>
        </li>
